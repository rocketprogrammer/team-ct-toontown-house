...
def getTargetUser(sock):
    ...
    if not os.path.isfile('/var/game/data/blobs/{0}.blob'.format(user)):
        # Create our user.
        open('/var/game/data/blobs/{0}.blob'.format(user), 'a').close()
    if user in tracker:
...
def newToon(acc,name,ia,spot,nToAp = None):
    _id = -1
    _toons = [int(t.split('.',1)[0].split('/')[-1]) for t in glob.glob('/var/game/data/toons/*.toon')]
    _toons.sort()
    ...
    if not ia and nToAp:
        with open('/var/game/data/toons/'+_id+'.nta','wb') as f:
            f.write('!'.join([nToAp,acc,spot]))
    
    with open('/var/game/data/toons/'+_id+'.toon','wb') as f:
...
def handler(sock,addr):
    ...
    ublob = Blob('/var/game/data/blobs/{0}.blob'.format(user))
